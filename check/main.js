document.addEventListener("DOMContentLoaded", () => {
    ...
    function replayPhysics(r) {
        if (!r.physics_valid) return;

        document.querySelector(
            `input[name="physics-valid"][value="${r.physics_valid}"]`
        ).checked = true;

        if (r.physics_valid === "false") {
            physicsReasonDiv.style.display = "block";

            r.physics_reasons.forEach(reason => {
                const cb = document.querySelector(
                    `input[name="physics-reason"][value="${reason}"]`
                );
                if (cb) cb.checked = true;
            });

            if (r.material_category) {
                document.querySelector(
                    `input[name="material-category"][value="${r.material_category}"]`
                ).checked = true;
                materialPanel.style.display = "block";
            }
        }
    }
    function replayKinematics(r) {
        if (!r.kinematics_valid) return;

        document.querySelector(
            `input[name="kinematics-valid"][value="${r.kinematics_valid}"]`
        ).checked = true;

        if (r.kinematics_valid === "false") {
            kinematicsReasonDiv.style.display = "block";

            r.kinematics_reasons.forEach(reason => {
                const cb = document.querySelector(
                    `input[name="kinematics-reason"][value="${reason}"]`
                );
                if (cb) cb.checked = true;
            });
            ...
            if (r.motion_type) {
                document.querySelector(
                    `input[name="motion-type"][value="${r.motion_type}"]`
                ).checked = true;
            }
            if (r.axis_choice !== null) {
                document.querySelector(
                    `input[name="axis-choice"][value="${r.axis_choice}"]`
                ).checked = true;
            }
            if (r.position_choice !== null) {
                document.querySelector(
                    `input[name="position-choice"][value="${r.position_choice}"]`
                ).checked = true;
            }
            if (r.motion_range) {
                motionStartInput.value = r.motion_range.start;
                motionEndInput.value = r.motion_range.end;
            }
        }
    }
    function replayResult(result) {
        replayPhysics(result);
        replayKinematics(result);
    }
    ...
    function loadData() {
        ...
        Promise.all([
            fetch(`/data/${sha256}`).then(r => r.json()),
            fetch(`/check/result/${sha256}`)
                .then(r => r.ok ? r.json() : null)
        ]).then(([raw, result]) => {
            renderRawData(raw);          // 你现在已有
            if (result) {
                replayResult(result);    // ⭐ 新增
            }
        }).catch(error => {
    ...
    function getCheckDecision() {
        const v = document.querySelector(
            "input[name='check-decision']:checked"
        )?.value;
        return v || null;
    }

    document.getElementById("submit-check-button").addEventListener("click", () => {
        const decision = getCheckDecision();
        if (!decision) {
            alert("请选择接受或拒绝选项再提交。");
            return;
        }
        ...
        const payload = {
            ...
            check_decision: decision,          // accept / reject
            check_reviewer_id: reviewerId,
        ...
        fetch("/check/submit", {
            ...
            .then(() => {
                // reset radio
                document.querySelectorAll("input[name='check-decision']")
    ...
    fetch("/check/ids/" + reviewerId + "/" + totalUserNum)
        .then(res => res.json())
        .then(list => {
            sha256List = list;
            currentIndex = 0;
